...
#define K1 K1_GABAb_S
 double K1 = 0.52;
#define exptemp exptemp_GABAb_S
 double exptemp = 34;
#define n n_GABAb_S
 double n = 4;
#define q10 q10_GABAb_S
 double q10 = 2.065;
#define temperature temperature_GABAb_S
 ...
 "temperature_GABAb_S", &temperature_GABAb_S,
 "q10_GABAb_S", &q10_GABAb_S,
 "exptemp_GABAb_S", &exptemp_GABAb_S,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 2;
 	...
 	ivoc_help("help ?1 GABAb_S C:/Users/dburka/Dropbox/shared/matlab/dburka/+NetworkSim/@Sim/gabab.mod\n");
 ...
 _tsav = t;   if (_lflag == 1. ) {*(_tqitem) = 0;}
 {
   if ( _lflag  == 0.0 ) {
     if (  ! _args[1] ) {
       ...
       lastrelease = t ;
       _args[1] = 1.0 ;
       net_send ( _tqitem, _args, _pnt, t +  Cdur + dt , 1.0 ) ;
       net_send ( _tqitem, _args, _pnt, t +  Cdur + Deadtime + dt , 2.0 ) ;
       }
     }
   if ( _lflag  == 1.0 ) {
     if ( C  == Cmax ) {
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   DR = K1 * phi_h * C * ( 1.0 - R ) - K2 * phi_h * R ;
   DG = K3 * R - K4 * G ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 DR = DR  / (1. - dt*( (K1 * phi_h * C)*(( ( - 1.0 ) )) - (K2 * phi_h)*(1.0) )) ;
 DG = DG  / (1. - dt*( ( - (K4)*(1.0) ) )) ;
 return 0;
}
 /*END CVODE*/
 static int bindkin (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    R = R + (1. - exp(dt*((K1 * phi_h * C)*(( ( - 1.0 ) )) - (K2 * phi_h)*(1.0))))*(- ( (((K1)*(phi_h))*(C))*(( 1.0 )) ) / ( (((K1)*(phi_h))*(C))*(( ( - 1.0) )) - ((K2)*(phi_h))*(1.0) ) - R) ;
