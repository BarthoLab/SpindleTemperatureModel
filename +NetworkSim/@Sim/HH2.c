...
#define kshift kshift_hh2
 double kshift = 0;
#define nashift nashift_hh2
 double nashift = 0;
 ...
 "kshift_hh2", &kshift_hh2,
 "nashift_hh2", &nashift_hh2,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 6);
 	...
 	ivoc_help("help ?1 hh2 C:/Users/dburka/Dropbox/shared/matlab/dburka/+NetworkSim/@Sim/HH2.mod\n");
...
static int  evaluate_fct (  double _lv ) {
   double _la , _lb , _lv2 ;
 _lv2 = _lv - vtraub ;
   _la = 0.32 * vtrap ( _threadargscomma_ 13.0 - ( _lv2 + nashift ) , 4.0 ) ;
   _lb = 0.28 * vtrap ( _threadargscomma_ ( _lv2 + nashift ) - 40.0 , 5.0 ) ;
   tau_m = 1.0 / ( _la + _lb ) / tadj ;
   m_inf = _la / ( _la + _lb ) ;
   _la = 0.128 * Exp ( _threadargscomma_ ( 17.0 - ( _lv2 + nashift ) ) / 18.0 ) ;
   _lb = 4.0 / ( 1.0 + Exp ( _threadargscomma_ ( 40.0 - ( _lv2 + nashift ) ) / 5.0 ) ) ;
   tau_h = 1.0 / ( _la + _lb ) / tadj ;
   h_inf = _la / ( _la + _lb ) ;
   _la = 0.032 * vtrap ( _threadargscomma_ 15.0 - ( _lv2 + kshift ) , 5.0 ) ;
   _lb = 0.5 * Exp ( _threadargscomma_ ( 10.0 - ( _lv2 + kshift ) ) / 40.0 ) ;
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 71 in file HH2.mod:\n	SOLVE states\n"); nrn_complain(_p); abort_run(error);}
