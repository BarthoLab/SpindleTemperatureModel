...
#define temperature temperature_it2
 ...
 "temperature_it2", &temperature_it2,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 it2 C:/Users/dburka/Dropbox/shared/matlab/dburka/+NetworkSim/@Sim/IT2.mod\n");
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  m = m0;
 {
   phi_m = pow( qm , ( ( temperature - 24.0 ) / 10.0 ) ) ;
   phi_h = pow( qh , ( ( temperature - 24.0 ) / 10.0 ) ) ;
   evaluate_fct ( _threadargscomma_ v ) ;
   m = m_inf ;
   h = h_inf ;
   }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   carev = ( 1e3 ) * ( R * ( temperature + 273.15 ) ) / ( 2.0 * FARADAY ) * log ( cao / cai ) ;
